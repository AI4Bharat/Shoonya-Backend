        ...
        if (
            ...
            for index, each_user in enumerate(users_ids):
                ...
                all_tasks_in_project = Task.objects.filter(
                    Q(project_id=pk) & Q(annotation_users=each_user)
                )
                ...
                annotated_tasks_objs = Task.objects.filter(
                    project_id=pk, annotation_users=each_user, task_status="accepted"
                )
                annotated_tasks_objs_ids = list(
                    annotated_tasks_objs.values_list("id", flat=True)
                )

                accepted_annotated_objs = Annotation_model.objects.filter(
                ...
                if proj.enable_task_reviews:
                    ...
                    accepted_wt_tasks_objs = Task.objects.filter(
                        project_id=pk,
                        annotation_users=each_user,
                        task_status="accepted_with_changes",
                    )
                    accepted_wt_tasks_objs_ids = list(
                        accepted_wt_tasks_objs.values_list("id", flat=True)
                    )

                    accepted_wt_change_objs = Annotation_model.objects.filter(
                        task_id__in=accepted_wt_tasks_objs_ids,
                        parent_annotation_id=None,
                        updated_at__range=[start_date, end_date],
                    )
                    ...
                    labeled_tasks_objs = Task.objects.filter(
                        project_id=pk, annotation_users=each_user, task_status="labeled"
                    )
                    labeled_tasks_objs_ids = list(
                        labeled_tasks_objs.values_list("id", flat=True)
                    )

                    annotation_labeled_objs = Annotation_model.objects.filter(
                        task_id__in=labeled_tasks_objs_ids,
                        parent_annotation_id=None,
                        updated_at__range=[start_date, end_date],
                    )
                    ...
                    rejected_tasks_objs = Task.objects.filter(
                        project_id=pk,
                        annotation_users=each_user,
                        task_status="rejected",
                    )
                    rejected_tasks_objs_ids = list(
                        rejected_tasks_objs.values_list("id", flat=True)
                    )

                    annotate_rejected_objs = Annotation_model.objects.filter(
                        task_id__in=rejected_tasks_objs_ids,
                        parent_annotation_id=None,
                        updated_at__range=[start_date, end_date],
                    )
                ...
                total_unlabeled_tasks = Task.objects.filter(
                    project_id=pk, annotation_users=each_user, task_status="unlabeled"
                )
                total_unlabeled_tasks_count = total_unlabeled_tasks.count()

                items.append(("Unlabeled Tasks", total_unlabeled_tasks_count))

                total_skipped_tasks = Task.objects.filter(
                    project_id=pk, annotation_users=each_user, task_status="skipped"
                )
                total_skipped_tasks_count = total_skipped_tasks.count()

                items.append(("Skipped Tasks", total_skipped_tasks_count))

                total_draft_tasks = Task.objects.filter(
                    project_id=pk, annotation_users=each_user, task_status="draft"
                )
                ...
                if is_translation_project:
                    if proj.enable_task_reviews:
                        all_annotated_tasks = (
                            list(accepted_annotated_objs)
                            + list(accepted_wt_change_objs)
                            + list(annotation_labeled_objs)
                            + list(annotate_rejected_objs)
                        )
                        ...
                        total_word_count_list = [
                            no_of_words(each_task.task.data["input_text"])
                            for each_task in accepted_annotated_objs
                        ]
                        total_word_count = sum(total_word_count_list)
                ...
                if proj.enable_task_reviews:
                    ...
                    lead_time_annotated_tasks = [
                        eachtask.lead_time for eachtask in accepted_annotated_objs
                    ]
                avg_lead_time = 0
            ...
            if proj.enable_task_reviews:
                ...
                rejected_tasks_objs = Task.objects.filter(
                    project_id=pk, annotation_users=each_user, task_status="rejected"
                )
                rejected_tasks_objs_ids = list(
                    rejected_tasks_objs.values_list("id", flat=True)
                )
