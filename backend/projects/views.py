...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def manual_task_assignment(self, request, pk):
        """
        Manually assign tasks to users for annotation/review/supercheck.
        """
        project = Project.objects.get(id=pk)
        task_ids = request.data.get("task_ids", [])
        user_id = request.data.get("user_id")
        ...
        for task in tasks:
            # ✅ Enforce proper assignment sequence
            if allocation_type == 2 and task.task_status != "annotated":
                continue  # Review can only be assigned after annotation
            elif allocation_type == 3 and task.task_status != "reviewed":
                continue  # Supercheck only after review

            # ✅ Assign user based on allocation_type
            if allocation_type == 1:
                task.annotation_users.add(user)
            elif allocation_type == 2:
                task.review_user = user
            ...
            # ✅ Create annotation object if not exists
            existing = Annotation_model.objects.filter(
                task_id=task,
                annotation_type=allocation_type,
                completed_by=user,
