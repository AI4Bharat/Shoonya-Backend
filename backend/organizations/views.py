        ...
        if tgt_language == None:
            ...
            users_objs_all_task_count = (
                ...
                .annotate(
                    total_tasks=Count(
                        "annotation_users",
                        filter=Q(
                            annotation_users__project_id__project_type=project_type
                        ),
                    )
                )
            )

            users_objs_labeled_task_count = (
                User.objects.filter(organization=organization)
                .order_by("username")
                .annotate(
                    accepted_tasks=Count(
                        "annotation",
                        filter=Q(
                            annotation__task__task_status__in=[
                                "accepted",
                                "rejected",
                        ...
                        & Q(annotation__task__project_id__project_type=project_type)
                        & Q(annotation__parent_annotation_id=None)
                        & Q(annotation__created_at__range=[start_date, end_date]),
                    )
                )
            )

            users_objs_skipped_task_count = (
                User.objects.filter(organization=organization)
                .order_by("username")
                .annotate(
                    skipped_tasks=Count(
                        "annotation_users",
                        filter=Q(annotation_users__task_status="skipped")
                        & Q(annotation_users__project_id__project_type=project_type),
                    )
                )
            )

            users_objs_draft_task_count = (
                User.objects.filter(organization=organization)
                .order_by("username")
                .annotate(
                    draft_tasks=Count(
                        "annotation_users",
                        filter=Q(annotation_users__task_status="draft")
                        & Q(annotation_users__project_id__project_type=project_type),
                    )
                )
            )
            users_objs_unlabeled_task_count = (
                User.objects.filter(organization=organization)
                .order_by("username")
                .annotate(
                    unlabeled_task=Count(
                        "annotation_users",
                        filter=Q(annotation_users__task_status="unlabeled")
                        & Q(annotation_users__project_id__project_type=project_type),
                    )
                )
            )
            ...
            if is_translation_project:
                for user in users:
                    ...
                    annotated_task_ids = list(
                        annotated_tasks.values_list("id", flat=True)
                    )
                    annotated_labeled_tasks = Annotation.objects.filter(
                        task_id__in=annotated_task_ids,
                        parent_annotation_id=None,
            ...
            for i in range(len(users)):
                ...
                result.append(
                    (
                        "No. of Workspaces",
                        users_objs_wt_workspace_count[i].no_of_workspaces,
                    )
                )
                result.append(
                    ("No. of Projects", users_objs_wt_proj_count[i].no_of_projects)
                )
                result.append(("Project Type", project_type))
                result.append(
                    ("No.Of Tasks Assigned", users_objs_all_task_count[i].total_tasks)
                )
                result.append(
                    (
                        "No. of Annotated Tasks",
                        users_objs_labeled_task_count[i].accepted_tasks,
                    )
                )
                result.append(
                    (
                        "Unlabeled Tasks",
                        users_objs_unlabeled_task_count[i].unlabeled_task,
                    )
                )
                result.append(
                    ("Skipped Tasks", users_objs_skipped_task_count[i].skipped_tasks)
                )
                result.append(
                    ("Draft Tasks", users_objs_draft_task_count[i].draft_tasks)
                )
                if is_translation_project:
