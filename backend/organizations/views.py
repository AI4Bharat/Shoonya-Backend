...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def user_analytics(self, request, pk=None):
        ...
        for user in users:
            ...
            total_skipped_tasks = total_no_of_tasks_assigned.filter(
                task_status="skipped"
            )
            total_skipped_tasks_count = total_skipped_tasks.count()

            total_unlabeled_tasks = total_no_of_tasks_assigned.filter(
        ...
        if (
            not is_translation_project
            and sort_by_column_name == "Word Count Of Annotated Tasks"
        ):
            return Response(
                {
