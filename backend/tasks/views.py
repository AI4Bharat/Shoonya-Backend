...
from tasks.models import Task, TaskLock, Annotation, Prediction
...
class TaskViewSet(viewsets.ModelViewSet, mixins.ListModelMixin):
    ...
    # @action(detail=True, methods=["get"], url_path="list_for_review")
    # def list_for_review(self, request, pk, *args, **kwargs):
    #     print(kwargs)
    ...
    #         if queryset.exists():
    #             queryset = queryset.filter(review_user=request.user.id)
    #     else:
    #         queryset = Task.objects.all()
...
class ReviewViewSet(
    ...
    def list(self, request, *args, **kwargs):
        queryset = Task.objects.filter(is_reviewable=True)
        user = request.user
        userRole = user.role if user.is_authenticated else -1
        
        print(f"User is authenticated? {userRole}")

        if userRole not in [WORKSPACE_MANAGER, ORGANIZATION_OWNER] and userRole > 0:
            queryset = queryset.filter(review_user=user)
        elif userRole < 0:
            queryset = queryset.none()

        if userRole == WORKSPACE_MANAGER:
            concerned_workspaces = Workspace.objects.filter(users_user_id__contains=user.user_id)
            queryset = queryset.filter(
                project_id__workspace_id__in=concerned_workspaces
            )

        if userRole == ORGANIZATION_OWNER:
            queryset = queryset.filter(project_id__organization_id=user.organization)
