...
class UserViewSet(viewsets.ViewSet):
    ...
    def user_profile_image_update(self, request, pk=None):
        try:
            ...
            try:
                blob = BlobClient.from_connection_string(conn_str=os.getenv("STORAGE_ACCOUNT_CONNECTION_STRING"), container_name=os.getenv("STORAGE_ACCOUNT_PROFILE_IMAGE_CONTAINER_NAME"), blob_name=file_upload_name)
                blob.upload_blob(BytesIO(file.read()),overwrite=True)
            except:
                return Response(
                    {"message": "Could not connect to azure blob storage"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
